...
const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });// Get the specific generative model to use for generating responses
...
app.post('/parse-outline', async (req, res) => {
    ...
    try 
    {
        ...
        const AItext = responseText.text();
        const weights = JSON.parse(AItext); // parse the response text JSON as JavaScript object

        res.json({ weights }); // send the parsed weights back to the client as JSON
    }
    catch (err) {
